...
export default function Drivers() {
  ...
  const renderGridItem = (driver: any) => (
    <div key={driver.id} className="border border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        ...
        <StatusBadge status={driver.status} colorMap={statusColors} />
      </div>

      <h3 className="font-bold text-lg text-gray-900 mb-1">{driver.name}</h3>
      <p className="text-sm text-gray-600 mb-3">{driver.email}</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Drivers"
        description={`Manage all drivers in the system (${drivers.length} drivers)`}
        action={
          <button
      ...
      <DataTable
        ...
        emptyIcon={<Truck size={48} className="mx-auto text-gray-300 mb-4" />}
      />
    </div>
  );
}