...
import { PackagePlus, MapPin, Truck, Bike, XCircle } from 'lucide-react';
...
export default function CancelledOrders() {
  ...
  const renderGridItem = (order: any) => (
    <div key={order.id} className="border border-red-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-red-50/30">
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`Orders that were cancelled (${cancelledOrders.length} orders)`}
        action={
          <button
      ...
      <DataTable
        ...
        emptyIcon={<XCircle size={48} className="mx-auto text-red-300 mb-4" />}
      />
    </div>
  );
}