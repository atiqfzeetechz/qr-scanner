...
import { Search, PackagePlus, MapPin, Grid3X3, List, MoreVertical, Truck, Bike, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`Orders waiting to be processed (${pendingOrders.length} orders)`}
        action={
          <button
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {filteredOrders.map((order) => (
              <div
                key={order.id}
                className="border border-yellow-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-yellow-50/30"
              >
